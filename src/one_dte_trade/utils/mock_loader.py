    ...
    def __init__(self, data_paths: Dict[str, str]):
        ...
        for source_name, path in self.data_paths.items():
            if not os.path.exists(path):
                print(
                    f"MockCatLoader Warning: Parquet file for source '{source_name}' not found at '{path}'. Method calls will fail."
                )
    ...
    def _load_source(self, source_name: str) -> pl.DataFrame:
        ...
        if source_name in self._loaded_data:
            return self._loaded_data[
                source_name
            ].clone()  # Return a clone to prevent accidental modification

        path = self.data_paths.get(source_name)
        if path is None:
            raise ValueError(
                f"MockCatLoader Error: No path configured for data source '{source_name}'."
            )
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"MockCatLoader Error: Parquet file for '{source_name}' not found at '{path}'."
            )
        ...
        try:
            ...
            raise IOError(
                f"MockCatLoader Error: Failed to read Parquet file '{path}' for source '{source_name}'. Error: {e}"
            )
